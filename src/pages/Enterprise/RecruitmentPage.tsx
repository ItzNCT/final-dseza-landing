          ...
          <main className="flex-1 px-4 py-4 space-y-4">
            ...
            <div className={`${isDark ? 'bg-dseza-dark-secondary/30' : 'bg-dseza-light-secondary/30'} rounded-lg px-2 py-1`}>
              <nav className={`flex items-center space-x-1 text-xs ${secondaryTextClass}`}>
                ...
                <span className={`font-medium ${textClass}`}>{language === 'en' ? 'Recruitment' : 'Tuyển dụng'}</span>
              </nav>
            </div>

            {/* Header - Mobile */}
            <div className="text-center">
              <h1 className={`text-xl font-bold ${textClass}`}>{language === 'en' ? 'Career Opportunities' : 'Cơ hội nghề nghiệp'}</h1>
              <div className={`w-12 h-0.5 mx-auto mt-2 rounded-full ${isDark ? 'bg-dseza-dark-primary' : 'bg-dseza-light-primary'}`} />
              <p className={`text-xs mt-2 ${secondaryTextClass}`}>{language === 'en' ? 'Job opportunities at Danang Hi-Tech Park' : 'Cơ hội việc làm tại Khu Công nghệ cao Đà Nẵng'}</p>
            </div>
            ...
            {isError && (
              <Alert className="mb-2">
                <AlertDescription>
                  {language === 'en' ? 'An error occurred while loading recruitment postings: ' : 'Có lỗi xảy ra khi tải tin tuyển dụng: '}{error?.message || (language === 'en' ? 'Unknown error' : 'Lỗi không xác định')}
                </AlertDescription>
              </Alert>
            )}
            ...
            {isLoading && (
              <div className="flex justify-center items-center py-10">
                <div className="text-center">
                  ...
                  <p className={`mt-3 text-xs ${secondaryTextClass}`}>{language === 'en' ? 'Loading recruitment list...' : 'Đang tải danh sách tuyển dụng...'}</p>
                </div>
              </div>
            )}
            ...
            {!isLoading && !isError && (
              jobs && jobs.length > 0 ? (
                    ...
                    <div key={job.id} className={`${cardClass} ${borderClass} border rounded-lg p-3`}>
                      <div className="flex justify-between items-start gap-3">
                        <div className="flex-1">
                          <h2 className={`text-base font-semibold mb-1 ${textClass}`}>{job.position || (language === 'en' ? 'Job position' : 'Vị trí tuyển dụng')}</h2>
                          {job.deadline && (
                            <p className={`text-xs flex items-center ${secondaryTextClass}`}>
                              <Calendar className="w-3.5 h-3.5 mr-1" /> {language === 'en' ? 'Application deadline' : 'Hạn nộp hồ sơ'}: {job.deadline}
                            </p>
                          )}
                        </div>
                        {job.salary && (
                      ...
                      <div className={`mt-3 p-2 rounded ${isDark ? 'bg-dseza-dark-main-bg' : 'bg-dseza-light-main-bg'} ${borderClass} border`}>
                        <p className={`text-xs ${secondaryTextClass}`}>
                          {language === 'en' ? 'Contact' : 'Liên hệ'}: <span className={`${isDark ? 'text-dseza-dark-primary' : 'text-dseza-light-primary'} font-medium`}>tuyendung@dseza.danang.gov.vn</span>
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                <div className={`rounded-lg ${cardClass} ${borderClass} border p-10 text-center`}>
                  ...
                  <h3 className={`text-base font-semibold mb-2 ${textClass}`}>{language === 'en' ? 'Currently no job postings' : 'Hiện tại không có tin tuyển dụng'}</h3>
                  <p className={`text-xs ${secondaryTextClass}`}>{language === 'en' ? 'We will update the latest recruitment information here. Please check back later.' : 'Chúng tôi sẽ cập nhật thông tin tuyển dụng mới nhất tại đây. Vui lòng quay lại sau.'}</p>
                </div>
              )
            )}
          </main>
          ...
          <div className="container mx-auto px-4">
            <nav className={`flex items-center space-x-2 text-sm ${secondaryTextClass}`}>
              ...
              <span className={`font-medium ${textClass}`}>
                {language === 'en' ? 'Recruitment' : 'Tuyển dụng'}
              </span>
            </nav>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="mb-12 text-center">
            <h1 className={`text-4xl md:text-5xl font-bold mb-4 ${textClass}`}>
              {language === 'en' ? 'Career Opportunities' : 'Cơ hội nghề nghiệp'}
            </h1>
            ...
            <div className="flex items-center justify-center space-x-2 text-lg">
              ...
              <span className={secondaryTextClass}>
                {language === 'en' ? 'Job opportunities at Danang Hi-Tech Park' : 'Cơ hội việc làm tại Khu Công nghệ cao Đà Nẵng'}
              </span>
            </div>
            <p className={`text-sm mt-4 max-w-2xl mx-auto ${secondaryTextClass}`}>
              {language === 'en' ? 'Join a professional, dynamic team and develop your career with leading technology companies.' : 'Tham gia đội ngũ chuyên nghiệp, năng động và phát triển sự nghiệp cùng các doanh nghiệp công nghệ hàng đầu.'}
            </p>
          </div>
            ...
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center space-x-4">
                <div className={`px-3 py-1 text-sm rounded-md ${isDark ? 'bg-dseza-dark-primary/20 text-dseza-dark-primary border border-dseza-dark-primary/30' : 'bg-dseza-light-primary/20 text-dseza-light-primary border border-dseza-light-primary/30'}`}>
                  {jobs.length} {language === 'en' ? (jobs.length === 1 ? 'job opening' : 'job openings') : 'vị trí tuyển dụng'}
                </div>
              </div>
            </div>
          )}
            ...
            <Alert className="mb-8">
              <AlertDescription>
                ...
                <span className="text-sm opacity-75">
                  {language === 'en' ? 'Please try again later or contact the administrator.' : 'Vui lòng thử lại sau hoặc liên hệ với quản trị viên.'}
                </span>
              </AlertDescription>
            </Alert>
          )}
          ...
          {isLoading ? (
                      ...
                      <div className="flex justify-between items-center w-full">
                        <div className="text-left flex-1 pr-4">
                          <h2 className={`text-xl font-bold mb-2 ${textClass}`}>
                            {job.position || (language === 'en' ? 'Job position' : 'Vị trí tuyển dụng')}
                          </h2>
                          {job.deadline && (
                            <p className={`text-sm flex items-center ${secondaryTextClass}`}>
                              <Calendar className="w-4 h-4 mr-2" />
                              {language === 'en' ? 'Application deadline' : 'Hạn nộp hồ sơ'}: {job.deadline}
                            </p>
                          )}
                        </div>
                        {job.salary && (
                    ...
                    <AccordionContent className="px-6 pb-6 pt-0">
                      <div className={`border-t pt-4 ${borderClass}`}>
                        ...
                        {job.requirements && (
                          <>
                            <h3 className={`font-semibold mb-3 ${textClass}`}>
                              {language === 'en' ? 'Candidate Requirements' : 'Yêu cầu ứng viên'}
                            </h3>
                            <div 
                        ...
                        <div className={`mt-6 p-4 rounded-lg ${isDark ? 'bg-dseza-dark-main-bg' : 'bg-dseza-light-main-bg'} ${borderClass} border`}>
                          ...
                          <p className={`text-sm ${secondaryTextClass}`}>
                            {language === 'en' ? 'To apply for this position, please send your application to email:' : 'Để ứng tuyển vị trí này, vui lòng gửi hồ sơ về email:'} 
            ...
            <div className={`rounded-lg ${cardClass} ${borderClass} border p-16 text-center`}>
              ...
              <h3 className={`text-lg font-semibold mb-2 ${textClass}`}>
                {language === 'en' ? 'Currently no job postings' : 'Hiện tại không có tin tuyển dụng'}
              </h3>
              <p className={secondaryTextClass}>
                {language === 'en' ? 'We will update the latest recruitment information here. Please check back later.' : 'Chúng tôi sẽ cập nhật thông tin tuyển dụng mới nhất tại đây. Vui lòng quay lại sau.'}
              </p>
            </div>
          )}
          ...
          <div className="mt-16 text-center">
            <Link 
              to={language === 'en' ? '/enterprises' : '/doanh-nghiep'}
            ...
            >
              ← {language === 'en' ? 'Back to Business' : 'Quay lại Doanh nghiệp'}
            </Link>
          </div>
        </div>
      </main>
